...
main(int argc, char* argv[])
{
    ...
    properties->parseCommandLineOptions("Greeter", args);
    ...
    int maxConnections = communicator->getProperties()->getPropertyAsInt("Greeter.Connections");

    for (int i = 0; i < maxConnections; ++i)
    {
        greeter = greeter->ice_connectionId(to_string(i));
    ...
    cerr << "done, press any key to exit" << endl;
    char c;
    cin.get(c);
    return 0;
}
